...
# LLMNR POISONING AND SMB RELAY ATTACKS ON ACTIVE DIRECTORY
 ...
 *Ref 3: Onces you quick search the attacker IP address  it will prompt network credential login simply ignore those and immediately check the attack machine for response*
 - Onces that connection has been established ahash will be generated
...
## Step: SMB Relay
- To perform SMB relay, identify the smbhost from the DC machine using nmap 
...
- The other option was to run through the interactive mode thus –I to produce a client shell
...
- With enumaration, used metaspoilt >psexec > exploit/windows/smb/psexec, using this command to  execute commands on remote Windows systems. It leverages the Windows SMB (Server Message Block) protocol to authenticate and execute commands on remote systems
...
- Based on options performed payload update
 ...
 *Ref 6:  This also points out the module used as windows/smb/psexec*
 ...
 - Similarly, set up all the other requirements like rhost, subdomain, smbuser and smbpass as illustrated below
 ...
 - Then performed an NTML attack using the gained session thus hash attack but first unset the subdomain then set the smbuse as administrator, similarly called the out half session of the hash generated from responder earlier and set it as smbpass
 ...
 after the set up executed the attack and thus the output
 ...
 - Then  attempted to gain access to shell remotely using psexec.py with punisher machine and its password and IP address
